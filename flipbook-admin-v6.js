...
function goToStep(stepNumber) {
    ...
    // Step 3 is now Reorder Pages - display the grid
    if (stepNumber === 3) {
        displayPageReorderGrid();
    }

    // Step 5 is now Assign Audio - update the assignment list
    if (stepNumber === 5) {
...
async function saveFlipbookInBatches(title, description, orientation, audioAssignments) {
    try {
        ...
        // Check if we're editing an existing flipbook
        if (currentFlipbookId) {
            // Editing mode - use existing ID
            flipbookId = currentFlipbookId;
            console.log(`Updating existing flipbook with ID: ${flipbookId}`);
        } else {
            // Create new flipbook - Step 1: Create flipbook with just 1 page (to get flipbook ID)
            console.log('Step 1: Creating new flipbook...');
            const initialData = {
                title: title,
                description: description,
                orientation: orientation,
            ...
            flipbookId = createResult.flipbookId;
            console.log(`Flipbook created with ID: ${flipbookId}`);
        }

        // Step 2: Upload pages in batches of 20
        const BATCH_SIZE = 20;
        ...
        for (let i = 0; i < pagesToUpload.length; i += BATCH_SIZE) {
            const batch = pagesToUpload.slice(i, Math.min(i + BATCH_SIZE, pagesToUpload.length));
...
function copyEmbedCode(type, event) {
    let elementId;
    if (type === 'iframe') {
